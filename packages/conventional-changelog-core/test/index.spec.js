...
setups([
  () => { // 1
    ...
    testTools.writeFileSync('package.json', JSON.stringify({
      ...
      repository: {
        type: 'git',
        url: 'https://github.com/nholuongut/conventional-changelog-core.git'
      }
    }))
...
describe('conventional-changelog-core', () => {
  ...
  it('should fallback to use repo url if repo is repository is null', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('](https://stash.local/scm/nholuongut/conventional-changelog/commits/')
      expect(chunk).toContain('closes [#1](https://stash.local/scm/nholuongut/conventional-changelog/issues/1)')
    }
  })
  ...
  it('should read the closest package.json by default', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('closes [#1](https://github.com/nholuongut/conventional-changelog-core/issues/1)')
    }
  })
