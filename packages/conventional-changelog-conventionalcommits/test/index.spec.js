...
setups([
  () => {
    ...
    testTools.writeFileSync('package.json', JSON.stringify({
      ...
      repository: {
        type: 'git',
        url: 'https://github.com/nholuongut/conventional-changelog.git'
      }
    }))
...
describe('conventional-changelog-conventionalcommits', () => {
  it('should work if there is no semver tag', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain(', closes [#1](https://github.com/nholuongut/conventional-changelog/issues/1) [#2](https://github.com/nholuongut/conventional-changelog/issues/2)')
  ...
  it('should properly format external repository issues', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('[#1](https://github.com/nholuongut/conventional-changelog/issues/1)')
  ...
  it('should replace #[a-z0-9]+ with issue URL by default', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('[#133](https://github.com/nholuongut/conventional-changelog/issues/133)')
      expect(chunk).toContain('[#1a2b](https://github.com/nholuongut/conventional-changelog/issues/1a2b)')
    }
  })
  ...
  it('should remove the issues that already appear in the subject', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('[#88](https://github.com/nholuongut/conventional-changelog/issues/88)')
      expect(chunk).not.toContain('closes [#88](https://github.com/nholuongut/conventional-changelog/issues/88)')
    }
  })
  ...
  it('should fallback to the closest package.json when not providing a location for a package.json', async () => {
    ...
    for await (let chunk of conventionalChangelogCore({
      ...
      expect(chunk).toContain('(https://github.com/nholuongut/conventional-changelog/compare')
      expect(chunk).toContain('](https://github.com/nholuongut/conventional-changelog/commit/')
      expect(chunk).toContain('](https://github.com/nholuongut/conventional-changelog/issues/')
      i++
    }
